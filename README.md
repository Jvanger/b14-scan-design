...
1. Modify the percentage value in the `synthesis_pscan.tcl` script:
   ```tcl
   # Original line (for K=86, 40% of flip-flops)
...
2. Run synthesis with the modified script:
   ```
   design_vision -no_gui -f synthesis_pscan.tcl
   ```

3. Check the generated reports to extract key metrics:
